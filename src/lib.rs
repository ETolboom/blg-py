...
fn analyze_safeness(model: &str) -> PyResult<PyProperty> {
    match read_bpmn_from_string(model) {
        Ok(collaboration) => {
            let mut property_result = run(&collaboration, vec![Property::Safeness]);
...
fn analyze_dead_activities(model: &str) -> PyResult<PyProperty> {
    match read_bpmn_from_string(model) {
        Ok(collaboration) => {
            let mut property_result = run(&collaboration, vec![Property::NoDeadActivities]);
...
fn analyze_option_to_complete(model: &str) -> PyResult<PyProperty> {
    match read_bpmn_from_string(model) {
        Ok(collaboration) => {
            let mut property_result = run(&collaboration, vec![Property::OptionToComplete]);
...
fn analyze_proper_completion(model: &str) -> PyResult<PyProperty> {
    match read_bpmn_from_string(model) {
        Ok(collaboration) => {
            let mut property_result = run(&collaboration, vec![Property::ProperCompletion]);
